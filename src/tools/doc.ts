...
/**
 * Parses markdown text and returns styled HTML.
 */
export function convertMdToHtml(content: string): string {
...
/**
 * Uses MS Edge in headless mode to print HTML content to a PDF.
 */
export async function convertHtmlToPdf(htmlContent: string, pdfPath: string): Promise<boolean> {
...
export async function convertMdToPdf(mdPath: string, pdfPath: string): Promise<boolean> {
  ...
  try {
    ...
    const htmlContent = convertMdToHtml(content);

    const success = await convertHtmlToPdf(htmlContent, pdfPath);
    if (success) {
